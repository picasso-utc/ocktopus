...
<p>Nous t'envoyons ce mail pour te dire que ta demande d'exté pour <strong>{{ $record->exte_nom_prenom }}</strong> du <strong> {{Carbon::parse($record->exte_date_debut)->translatedFormat('d F Y') }} au {{Carbon::parse($record->exte_date_fin)->translatedFormat('d F Y')  }}</strong></p>
